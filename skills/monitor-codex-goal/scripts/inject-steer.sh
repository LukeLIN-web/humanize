...
#   key     <pane> <keyname>      send ONE named control key (Escape/Up/Down/Enter/
#                                 a digit/...), with before+after evidence capture,
#                                 for deterministic menu navigation + popup dismiss
#                                 (the control-key half of the steer protocols).
#   interrupt <pane>              send Escape to interrupt a running tool call and
#                                 verify the "esc to interrupt" busy marker cleared
#                                 (wedge recovery).
#
# `key` and `interrupt` exist so EVERY keystroke to Codex -- not just free-text
# pastes -- flows through one audited, evidence-logged path. The skill's read-only
# contract bans raw `tmux send-keys`; these two subcommands are how the menu /
# interrupt / dismiss protocols honor that while staying deterministic. Both run
# verify-target first and refuse a non-Codex / dead / copy-mode pane. `key` sends
# exactly one keystroke per call (no auto-repeat) so the caller captures and
# verifies the effect after each press, never blind-pressing a sequence.
#
# Exit codes:
#   0  success
...
#   42 interrupt sent but the busy marker did not clear (still wedged)
...
usage() {
  ...
  err "       inject-steer.sh key <pane> <keyname>     (Escape|Up|Down|Left|Right|Enter|BTab|Tab|Space|Home|End|0-9|A-Za-z)"
  err "       inject-steer.sh interrupt <pane>"
  exit 64
}
...
# Busy marker: Codex shows an "Esc to interrupt" / "to interrupt" affordance (and
# a "Working"/"Esc to interrupt" footer) while a tool/turn is running. Its
# presence is the structural "busy" signal; its absence means the composer is idle.
_is_busy() { # $1=pane -> 0 if a running-tool/interrupt affordance is visible
  _capture "$1" | grep -qiE 'to interrupt|esc to interrupt' && return 0
  return 1
}

# Validate a tmux key NAME against a tight allowlist. We never forward arbitrary
# key strings: only navigation / confirm / dismiss keys and single alphanumerics
# (digit dismiss of a modal popup, a single-letter menu accelerator).
_valid_key() { # $1=keyname -> 0 if allowed
  case "$1" in
    Escape|Enter|Up|Down|Left|Right|BTab|Tab|Space|Home|End|PageUp|PageDown) return 0 ;;
    [0-9]|[A-Za-z]) return 0 ;;
    *) return 1 ;;
  esac
}

do_key() { # $1=pane $2=keyname -> send one key, save before/after, print tails
  local pane="$1" key="$2" before after
  if ! _valid_key "$key"; then
    err "refusing key '$key' -- not in the allowlist (Escape|Enter|Up|Down|Left|Right|BTab|Tab|Space|Home|End|PageUp|PageDown|0-9|A-Za-z)"
    return 64
  fi
  [ -n "$EVID" ] || _evidence_dir >/dev/null
  _meta "$pane" | _save target.txt
  before="$(_capture "$pane")"
  printf '%s' "$before" | _save before.txt
  # One keystroke only -- no -N repeat. The caller verifies the effect and decides
  # whether to send another, so a runaway sequence is impossible.
  if ! tmux send-keys -t "$pane" "$key" 2>/dev/null; then
    err "send-keys '$key' failed"; return 40
  fi
  sleep "$POLL"; sleep "$POLL"
  after="$(_capture "$pane")"
  printf '%s' "$after" | _save after-key.txt
  local delta=changed; [ "$before" = "$after" ] && delta=unchanged
  printf 'key-sent pane=%s key=%s pane-delta=%s\n' "$pane" "$key" "$delta"
  printf '%s\n' "--- after (tail) ---"
  printf '%s\n' "$after" | tail -n 12
  return 0
}

# ---- interrupt (Escape a wedged tool call) ----------------------------------

do_interrupt() { # $1=pane -> Escape, verify the busy marker cleared
  local pane="$1" i
  [ -n "$EVID" ] || _evidence_dir >/dev/null
  _meta "$pane" | _save target.txt
  _capture "$pane" | _save before.txt
  if ! _is_busy "$pane"; then
    # Nothing running to interrupt -- report it rather than firing Escape blindly.
    _capture "$pane" | _save after-interrupt.txt
    printf 'interrupt pane=%s state=already-idle (no busy marker; nothing to interrupt)\n' "$pane"
    return 0
  fi
  tmux send-keys -t "$pane" Escape 2>/dev/null || { err "send-keys Escape failed"; return 40; }
  for ((i=0; i<SUBMIT_TRIES; i++)); do
    sleep "$POLL"
    if ! _is_busy "$pane"; then
      _capture "$pane" | _save after-interrupt.txt
      printf 'interrupt pane=%s state=idle (busy marker cleared)\n' "$pane"
      return 0
    fi
  done
  _capture "$pane" | _save after-interrupt.txt
  err "interrupt sent but busy marker still present after $SUBMIT_TRIES polls -- do NOT hammer Escape; notify"
  return 42
}
