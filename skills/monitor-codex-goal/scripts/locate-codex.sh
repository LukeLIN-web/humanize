...
# Probe the server with `list-panes`, NOT `tmux info`: when this script runs from
# a non-attached context (a tool shell that is not a tmux client) `tmux info`
# exits 1 with "no current client" even though the server is up, producing a false
# "no tmux server" negative. `list-panes -a` needs only a running server.
if ! tmux list-panes -a -F '#{session_name}' >/dev/null 2>&1; then
