...
# --- _valid_key: control-key allowlist for `key`/menu navigation -------------
# Only navigation/confirm/dismiss keys and single alphanumerics are forwardable;
# everything else (chords, multi-key strings, shell text) is refused so the
# `key` subcommand can never smuggle an arbitrary keystroke into Codex.
for k in Escape Enter Up Down Left Right BTab Tab Space Home End PageUp PageDown 0 9 a Z; do
  true_  "valid_key allows $k"  _valid_key "$k"
done
for k in "C-c" "M-x" "rm -rf /" "Up Down" "12" "" "Enter;ls" "Esc"; do
  false_ "valid_key rejects [$k]" _valid_key "$k"
done
